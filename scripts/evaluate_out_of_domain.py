...
def build_error_analysis_rows(gold_spans_by_doc, pred_spans_by_doc, texts_by_doc):
    ...
    for doc_id in sorted(gold_spans_by_doc.keys()):
        ...
        # Not exporting exact matches, because they are not errors.
        # # Same-label overlaps with different boundaries are exported as same-label boundary differences.
        for gold_index, gold_span in enumerate(gold_spans):
            ...
            used_gold_indices.add(gold_index) # what if there are 2 prediction spans matching one gold span?
            used_pred_indices.add(pred_index)

            exact_match = (
                gold_span["start"] == pred_span["start"]
                and gold_span["end"] == pred_span["end"]
                and text[gold_span["start"]:gold_span["end"]] == text[pred_span["start"]:pred_span["end"]] # ! try in main file
...
def main():
    ...
    print("Saved error-analysis export to:")
    print(ERROR_ANALYSIS_OUTPUT)
    print("Error-analysis rows:", len(error_rows))
